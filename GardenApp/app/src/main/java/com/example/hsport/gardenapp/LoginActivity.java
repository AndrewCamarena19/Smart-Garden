...
public class LoginActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button BtnStart = (Button) findViewById(R.id.StartUp);
        BtnStart.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://76.94.123.147:49180/#/Support"));
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean>
    {
        ...
        public Boolean LoginCheck() throws IOException {
                ...
                URL url = new URL("http://172.112.41.210:8110/php/Login&Garden.php?user=" + username + "&password=" + pass);
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setConnectTimeout(8000);
                InputStream in = new BufferedInputStream(conn.getInputStream());
                readStream(in);
                conn.disconnect();
                if(data.toString().equals("No User"))
                    return false;
                else
                {
                    check = "Logged in";
        ...
        protected void onPostExecute(final Boolean success)
        {
            ...
            if (success)
            {
                ...
                Intent nIntent = new Intent(LoginActivity.this, ViewGardens.class);
                nIntent.putExtra("Data", data.toString());
                nIntent.putExtra("Username", mEmail);
                nIntent.putExtra("Password", mPassword);
    ...
    public class UserRegisterTask extends AsyncTask<Void, Void, Boolean>
    {
        ...
        public void Register()
        {
            try
            {
                String username = mEmail;
                String pass = mPassword;
                URL url = new URL("http://172.112.41.210:8110/php/NewReg.php?user="+username+"&password="+pass);
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                InputStream in = new BufferedInputStream(conn.getInputStream());
                data = readStream(in);
                if(data.equalsIgnoreCase("[]")) data = "Username already taken";
                else data = "Account Created, Follow steps to set up PI to take first measurements then sign in";
